...
export default function NowPlaying() {
  ...
  return (
    <div className="flex items-center sm:flex-row mb-8 space-x-0 sm:space-x-2 w-full">
      {data?.songUrl ? (
        <div className="mr-2 sm:mr-2">
          <AnimatedBars />
        </div>
      ) : (
        <svg className="h-4 w-4 mr-2 sm:mr-0 ml-auto mt-[-2px]" viewBox="0 0 168 168">
          <path
            fill="#1ED760"
      ...
      <div className="inline-flex flex-row sm:flex-row w-full max-w-full truncate">
        ...
        <p className="capsize text-gray-500 dark:text-gray-500 max-w-max truncate">
          {truncate(data?.artist, 15) ?? 'Spotify'}
        </p>
      </div>
    </div>
  )
